...
export default function ClasseDetailPage() {
  ...
  const charger = useCallback(async () => {
    ...
    try {
      ...
      // Le nom de l'établissement ne sert qu'à la co-signature du certificat :
      // son échec ne doit pas empêcher la fiche de classe de s'afficher.
      ...
      <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--color-text-muted)', marginTop: 4 }}>
            {SCHOOL_LEVEL_LABELS[fiche.level] ?? fiche.level} · {actifs.length} élève
            {actifs.length > 1 ? 's' : ''} · {rattaches} compte{rattaches > 1 ? 's' : ''} lié
            {rattaches > 1 ? 's' : ''}
          </p>
        </div>
