...
/** Vue visuelle des statistiques globales — partagée entre la page admin et la page publique. */
export default function StatsView({ stats }: { stats: GlobalStatsData }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 16 }}>
        <StatCard label="Joueurs inscrits" value={stats.totalUsers.toLocaleString('fr-FR')} icon={<Users size={20} />} color="#FFBC40" />
        <StatCard label="Parties jouées" value={stats.totalGames.toLocaleString('fr-FR')} icon={<Gamepad2 size={20} />} color="#1F91D0" />
        <StatCard label="Programmes" value={stats.totalPrograms} icon={<Rocket size={20} />} color="#9B59B6" />
        <StatCard label="Partenaires" value={stats.totalPartners} icon={<Building2 size={20} />} color="#3FAE6B" />
      </div>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 16 }}>
          ...
          <StatCard label="Inscriptions programme" value={stats.totalEnrollments.toLocaleString('fr-FR')} icon={<UserCheck size={20} />} color="#9B59B6" />
      ...
      <div className="glass-card p-6">
        <h2 style={{ fontSize: 14, fontWeight: 600, marginBottom: 4, color: 'var(--color-text)' }}>Inscriptions — 30 derniers jours</h2>
