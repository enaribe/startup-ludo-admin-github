 ...
 *
 * Usage: npx tsx scripts/seed-ideation.ts
 * (or run via: node --loader ts-node/esm scripts/seed-ideation.ts)
 *
...
function targetCategory(id: string): string {
  const geographic = [
...
function missionCategory(id: string): string {
  const efficiency = [
...
const targetCards: IdeationCard[] = RAW_TARGETS.map((t) => ({
  id: t.id,
...
const missionCards: IdeationCard[] = RAW_MISSIONS.map((m) => ({
  id: m.id,
...
const sectorCards: IdeationCard[] = RAW_SECTORS.map((s) => ({
  id: s.id,
...
const allCards: IdeationCard[] = [...targetCards, ...missionCards, ...sectorCards];

const ideationDecks: IdeationDeck[] = [
